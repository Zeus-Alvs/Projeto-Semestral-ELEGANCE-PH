...
$stmt = $pdo->prepare("SELECT foto FROM usuario WHERE id = ?");
$stmt->execute([$id]);
$usuario = $stmt->fetch();
    ...
    if (!empty($_FILES["novafoto"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["novafoto"]["tmp_name"], $novo_nome)) {
            ...
            if ($stmt->execute([$novo_nome, $id])) {
                $_SESSION['usuario_foto'] = $novo_nome; // atualizar sessão
                echo '<script>alert("Foto alterada com sucesso!"); window.location.href="dashboard.php";</script>';
                exit;
            } else {
...
<div class="pagina-alterar-foto"> <h1>Alterar Foto de Perfil</h1>

    <?php if (!empty($erro)) { 
        echo "<div class='msg-erro'>$erro</div>"; 
    } ?>

    <form action="alterarfoto.php" method="post" enctype="multipart/form-data" class="form-upload">
        
        <div class="area-input-file">
            <label for="file-upload" class="label-arquivo">
                Escolha sua nova foto
            </label>
            <input id="file-upload" class="input-arquivo" type="file" name="novafoto" accept="image/*" required>
            <p class="dica-arquivo">Formatos: JPG, PNG, JPEG</p>
        </div>

        <input type="submit" value="Salvar Alteração" class="btn-salvar">
    </form>
